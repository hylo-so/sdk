...
/// pool, the pool balance it deposited into, and staleness (no harvest
/// yet for the current epoch).
#[derive(Debug, Clone, Copy)]
pub struct RealizedHarvest {
  pub epoch: u64,
  pub hyusd_to_pool: UFix64<N6>,
  /// Pool balance immediately BEFORE this harvest deposited into it,
  /// from `HarvestCache::stability_pool_cap` — the denominator that
  /// makes the realized rate equal the NAV step this harvest caused.
