...
/// Which stablecoin redeem fee a `HYUSD -> collateral` route prices with.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(super) enum RedeemFeeMode {
  /// The on-chain fee. Fails above the fee-curve domain.
  Strict,
  /// The fee at `min(projected CR, curve x_max)`. Not executable above
  /// the domain; see [`FeeBasis`].
  Clamped,
}
...
fn max_zero_exo<E: Exo>() -> Result<UFix64<N9>, CoreError>
where
  UFix64<E::Exp>: FixExt,
{
  UFix64::<E::Exp>::new(1)
...
impl<C: SolanaClock> ProtocolState<C> {
  ...
  /// `HYUSD -> LST` redeem math. In [`RedeemFeeMode::Clamped`] the fee
  /// is clamped at the curve domain edge, where the fee is flat and the
  /// marginal rate is the realized linear rate.
  ///
  /// # Errors
  /// * Conversion, liquidity, fee, burn limit, or marginal rate
  pub(super) fn redeem_stablecoin_lst_quote<L: LST + Local>(
    &self,
    in_amount: UFix64<N6>,
    mode: RedeemFeeMode,
    ...
    let (extract, basis) = match mode {
      RedeemFeeMode::Strict => self
        .exchange_context
      ...
      RedeemFeeMode::Clamped => {
        let projected = self
          .exchange_context
          .projected_redeem_state(&lst_price, lst_out)?;
        clamped_redeem_fee(
...
impl<C: SolanaClock> ProtocolState<C> {
  ...
  {
    let exo = &self.exo_pair::<E>()?.context;
    let max_zero_collateral = exo
      .exo_conversion()
      .max_exo_for_token(UFix64::zero(), exo.stablecoin_nav()?)?;
    let fee_rate = exo.stablecoin_mint_fee_rate(max_zero_collateral)?;
    let max_zero_in =
      FeeExtract::<N9>::max_input::<N5>(fee_rate, max_zero_collateral)?;
    past_zero(
      max_zero_in
        .checked_convert::<E::Exp>()
        .ok_or(CoreError::TokenAmountPrecision)?,
  ...
  /// `HYUSD -> Exo` redeem math. In [`RedeemFeeMode::Clamped`] the fee
  /// is clamped at the curve domain edge, where the fee is flat and the
  /// marginal rate is the realized linear rate.
  ///
  /// # Errors
  /// * Pair lookup, conversion, liquidity, fee, burn limit, or marginal
  pub(super) fn redeem_stablecoin_exo_quote<E: Exo + PythOracle>(
    &self,
    in_amount: UFix64<N6>,
    mode: RedeemFeeMode,
  ...
  fn redeem_stablecoin_exo_max_input<E: Exo + PythOracle>(
    ...
    let pair = self.exo_pair::<E>()?;
    let vault_cap = pair.context.exo_conversion().max_token_for_exo(
      pair.context.total_collateral,
      pair.context.stablecoin_nav()?,
    )?;
    let supply_cap = pair
      .context
      .virtual_stablecoin_supply()?
    ...
    UFix64<E::Exp>: FixExt,
  {
    ...
    let max_zero_collateral =
      FeeExtract::<N9>::max_input::<N5>(fee_rate, max_zero_exo::<E>()?)?;
    let max_zero_hyusd = exo
      .exo_conversion()
      .max_token_for_exo(max_zero_collateral, exo.stablecoin_nav()?)?;
    past_zero(max_zero_hyusd)
  }
}
...
impl<C: SolanaClock> ProtocolState<C> {
    ...
    UFix64<E::Exp>: FixExt,
  {
    ...
    FeeExtract::<N9>::max_input::<N4>(fee_rate, remaining)?
      .checked_convert::<E::Exp>()
      .ok_or(CoreError::TokenAmountPrecision)
  }
    ...
    UFix64<E::Exp>: FixExt,
  {
    ...
    let fee_rate = exo.levercoin_mint_fee_rate(max_zero_collateral)?;
    let max_zero_in =
      FeeExtract::<N9>::max_input::<N4>(fee_rate, max_zero_collateral)?;
    past_zero(
      max_zero_in
        .checked_convert::<E::Exp>()
        .ok_or(CoreError::TokenAmountPrecision)?,
    ...
    UFix64<E::Exp>: FixExt,
  {
    ...
    let max_zero_collateral =
      FeeExtract::<N9>::max_input::<N4>(fee_rate, max_zero_exo::<E>()?)?;
    let max_zero_levercoin = exo
      .exo_conversion()
      .max_token_for_exo(max_zero_collateral, exo.levercoin_redeem_nav()?)?;
    past_zero(max_zero_levercoin)
  }
}
...
impl<C: SolanaClock> ProtocolState<C> {
  ...
  fn convert_stable_to_lever_exo_quote<E: Exo + PythOracle>(
    ...
    let FeeExtract {
      fees_extracted,
      amount_remaining,
    } = pair.context.stablecoin_to_levercoin_fee(in_amount)?;
    let out_amount = pair
      .context
      .swap_conversion()?
      .stable_to_lever(amount_remaining)?;
  ...
  fn convert_stable_to_lever_exo_max_input<E: Exo + PythOracle>(
    ...
    let pair = self.exo_pair::<E>()?;
    let market_cap = pair.context.swap_conversion()?.max_stable_for_lever(
      pair
        .context
        .levercoin_market_cap_limiter()?
        .max_token_out()?,
    ...
    let burn_cap = supply.checked_sub(&pair.supply_floor).unwrap_or_default();
    let remaining = market_cap.min(burn_cap);
    let fee_rate = pair.context.stablecoin_to_levercoin_fee_rate(remaining)?;
    let projectable = pair.context.max_stablecoin_removal()?;
...
impl<C: SolanaClock> TokenOperation<JITOSOL, USDC> for ProtocolState<C> {
  ...
  fn preconditions(&self) -> Result<(), CoreError> {
    self.rebalance_buy_preconditions()
  }
...
impl<C: SolanaClock> TokenOperation<USDC, JITOSOL> for ProtocolState<C> {
  ...
  fn preconditions(&self) -> Result<(), CoreError> {
    self.rebalance_sell_preconditions()
  }
...
impl<C: SolanaClock> ProtocolState<C> {
    ...
    UFix64<E::Exp>: FixExt,
  {
    ...
    let max_zero_collateral = exo
      ...
      .max_exo_for_token(UFix64::zero(), UFix64::<N9>::one())?;
    past_zero(
      max_zero_collateral
        .checked_convert::<E::Exp>()
        .ok_or(CoreError::TokenAmountPrecision)?,
    ...
    UFix64<E::Exp>: FixExt,
  {
    ...
    past_zero(
      exo
        ...
        .max_token_for_exo(max_zero_exo::<E>()?, UFix64::<N9>::one())?,
    )
  }
}
  ...
  UFix64<E::Exp>: FixExt,
{
  ...
  fn compute_output_ungated(
    ...
    self
      .redeem_stablecoin_exo_quote::<E>(in_amount, RedeemFeeMode::Strict)
      .map(|(output, _)| output)
  }
  ...
  fn compute_output_indicative(
    ...
    self
      .redeem_stablecoin_exo_quote::<E>(in_amount, RedeemFeeMode::Clamped)
      .map(|(output, _)| output)
  }
...
impl<C: SolanaClock> TokenOperation<HYUSD, JITOSOL> for ProtocolState<C> {
  ...
  fn compute_output_ungated(
    ...
    self
      .redeem_stablecoin_lst_quote::<JITOSOL>(in_amount, RedeemFeeMode::Strict)
      .map(|(output, _)| output)
  }
  ...
  fn compute_output_indicative(
    ...
    self
      .redeem_stablecoin_lst_quote::<JITOSOL>(in_amount, RedeemFeeMode::Clamped)
      .map(|(output, _)| output)
  }
...
impl<C: SolanaClock> TokenOperation<HYUSD, HYLOSOL> for ProtocolState<C> {
  ...
  fn compute_output_ungated(
    ...
    self
      .redeem_stablecoin_lst_quote::<HYLOSOL>(in_amount, RedeemFeeMode::Strict)
      .map(|(output, _)| output)
  }
  ...
  fn compute_output_indicative(
    ...
    self
      .redeem_stablecoin_lst_quote::<HYLOSOL>(in_amount, RedeemFeeMode::Clamped)
      .map(|(output, _)| output)
  }
