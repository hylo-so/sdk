...
use hylo_idl::exchange::events::{
  RedeemLevercoinLstEvent, RedeemStablecoinLstEvent,
};
use hylo_idl::stability_pool::events::UserWithdrawEvent;
...
impl<L: LST + Local> BuildTransactionData<SHYUSD, L> for SimulationStrategy {
  ...
  async fn build(
    ...
    let withdraw_sim = self
      .stability_pool_client
      .simulate_transaction_event::<UserWithdrawEvent>(&withdraw_tx)
      .await?;
  ...
  for SimulationStrategy
{
  ...
  async fn get_quote(
    ...
    let from_hyusd: UFix64<N9> =
      parse_event::<RedeemStablecoinLstEvent>(&sim_result)
        .and_then(|e| e.collateral_withdrawn.try_into().map_err(Into::into))
        .unwrap_or_default();
    let from_xsol: UFix64<N9> =
      parse_event::<RedeemLevercoinLstEvent>(&sim_result)
        .and_then(|e| e.collateral_withdrawn.try_into().map_err(Into::into))
        .unwrap_or_default();
