...
/// Shard 0 holds the sponsored price account for every feed we consume.
...
pub const fn pyth_feed_address(feed_id: FeedId) -> Pubkey {
  let (key, _bump) = const_crypto::ed25519::derive_program_address(
    &[&PYTH_SHARD_0, &feed_id],
    pyth_solana_receiver_sdk::PYTH_PUSH_ORACLE_ID.as_array(),
  );
  Pubkey::new_from_array(key)
}

impl PythFeed {
  /// Builds a feed with its price account derived from `feed_id`.
  #[must_use]
...
mod tests {
  ...
  fn feed_addresses_derive_known_accounts() {
    ...
    .iter()
    .for_each(|(feed, known)| assert_eq!(feed.address, *known));
  }
}
